---
title: "Hypothesis testing via simulation <br> `r emo::ji('ab')`"
author: ""
output:
...
- If you're eligible to vote, but you haven't yet, do it!
- Peer evals due tonight -- must fill it out to receive your score
- Midterm this weekend, due next Tue
- Project proposals due following Tue (will update you on teams tomorrow morning after reviewing peer evals)
...
- Respnse variable is `outcome` in the `organ_donor` data frame
- This is a categorical variable with two levels, we're interested 
in `"complication"`s so we define that level as the success
...
- Two options for `null` hypothesis: `"point"` and `"independence"`, 
here we had $H_0: p = 0.10$, so that's a point value
- Then we state the hypothesized value of the success probability, 
which is 0.10. For completeness, we state the probability of failure 
as well.
...
- Then we generate 100 repetitions
- There are three possible generation `type`s infer uses:
  - `"bootstrap"` for estimation
  - `"permute"` when testing for independence and `"simulate"` when testing for a point null
...
- Finally calculate simulated sample statistics, in this case 
simulated `prop`ortions
...
https://www.youtube.com/watch?v=bCCCxV3nNgs
